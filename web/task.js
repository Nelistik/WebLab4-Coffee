class searchCoffie {
  notFoundCoffie = {
    "title": "Not found",
    "description": "There is no such coffee, but you can make it yourself at home",
    "ingredients": [
    ],
    "image": "./assets/5dKjBwyCKrQ",
    "id": 0
  }
  coffies = [
  ...
  constructor() {
    this.ingred = [];
    this.checkbox = Array.from(document.querySelectorAll(".ing_block"));
    this.checkbox.forEach(element => {
      element.querySelector(".checkbox").addEventListener("click", () => {
        if (element.querySelector(".clone__icon").classList.contains("clone__icon_checked")) {
          let ingrName = element.querySelector(".text__highlights").textContent;
          if (this.ingred.includes(ingrName)) {
            this.ingred.splice(this.ingred.indexOf(ingrName), 1)
            this.searching();
          }
          element.querySelector(".clone__icon").classList.remove("clone__icon_checked");
        } else {
          let ingrName = element.querySelector(".text__highlights").textContent;
          if (!this.ingred.includes(ingrName)) {
            this.ingred.push(ingrName);
            this.searching();
          }
          element.querySelector(".clone__icon").classList.add("clone__icon_checked");
        }
      })
    })
  }
  ...
  searching() {
    ...
    this.coffies.forEach((element) => {
      difference = this.ingred
        .filter(num => !element.ingredients.includes(num))
        .concat(element.ingredients.filter(num => !this.ingred.includes(num)));
      if (difference.length == 0) {
        document.querySelector(".coffee_image").style.cssText = `--src:url(${element.image})`;
    ...
    if (flag == 0) {
      document.querySelector(".coffee_image").style.cssText = `--src:url(https://i.pinimg.com/736x/5d/05/2b/5d052b26de17c0060928b2c7637fe81b.jpg)`;
      document.querySelector(".highlights2").textContent = this.notFoundCoffie.title;
      document.querySelector(".tEXT_THERE").textContent = this.notFoundCoffie.description;
    }
  }

}
const activeClass = new searchCoffie();